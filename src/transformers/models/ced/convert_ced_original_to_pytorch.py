...
from transformers import CedConfig, ASTFeatureExtractor, CedForAudioClassification
...
def rename_key(name):
    if "module.v" in name:
        name = name.replace("module.v", "ced")
    ...
    # final layernorm
    if "ced.norm" in name:
        name = name.replace("ced.norm", "ced.layernorm")
    # classifier head
...
def convert_state_dict(orig_state_dict, config):
    for key in orig_state_dict.copy().keys():
        ...
        if "qkv" in key:
            ...
            if "weight" in key:
                orig_state_dict[
                    f"ced.encoder.layer.{layer_num}.attention.attention.query.weight"
                ] = val[:dim, :]
                orig_state_dict[
                    f"ced.encoder.layer.{layer_num}.attention.attention.key.weight"
                ] = val[dim : dim * 2, :]
                orig_state_dict[
                    f"ced.encoder.layer.{layer_num}.attention.attention.value.weight"
                ] = val[-dim:, :]
            else:
                orig_state_dict[
                    f"ced.encoder.layer.{layer_num}.attention.attention.query.bias"
                ] = val[:dim]
                orig_state_dict[
                    f"ced.encoder.layer.{layer_num}.attention.attention.key.bias"
                ] = val[dim : dim * 2]
                orig_state_dict[
                    f"ced.encoder.layer.{layer_num}.attention.attention.value.bias"
                ] = val[-dim:]
        else:
